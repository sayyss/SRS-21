...
def tag():
    ...
    # create pd.Series, indexed by the task IDs (rows), filled with empty lists
    ...
    skill_df = pd.DataFrame(mlb.fit_transform(skill_series), columns=mlb.classes_, index=skill_series.index)
    skill_df.to_csv('data/tags.csv')
